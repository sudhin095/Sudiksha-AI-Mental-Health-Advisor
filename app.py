...
if not GEMINI_API_KEY:
    st.error("âš ï¸ No GEMINI_API_KEY found. Add it to .streamlit/secrets.toml.")
    st.stop()
        ...
        try:
            ...
            if "429" in msg or "quota" in msg or "rate limit" in msg:
                ...
                if model_id == "models/gemini-2.5-flash":
                    ...
                    return None
                # warn user once
                st.warning("âš ï¸ Model quota/rate limit reached. Switching to Gemini 2.5 Flash as fallback.")
                try:
...
def get_stress_desc(level):
    if level < 25: return "ðŸ˜Œ Minimal Stress â€” You seem calm."
    if level < 50: return "ðŸ™‚ Mild Stress â€” Manageable tension."
    if level < 75: return "ðŸ˜Ÿ Moderate Stress â€” Consider coping tools."
    return "ðŸ˜° High Stress â€” Strong distress detected."
...
- Immediate 12â€“24 hour plan (3 items)
...
<div style="
    ...
    font-size:0.95rem;
    font-style:italic;">
â€œIf you're going through hell, keep going.â€<br>
â€œIf there is something that means a lot to you, do not postpone it.â€
</div>
""", unsafe_allow_html=True)
...
with col1:
    tab1, tab2 = st.tabs(["âœï¸ Text", "ðŸŽ¤ Voice"])
    input_text = ""
    ...
    with tab2:
        ...
        audio_data = mic_recorder(start_prompt="ðŸŽ¤ Start Recording", stop_prompt="â¹ Stop")
        if audio_data:
            st.audio(audio_data["bytes"], format="audio/wav")
            st.success("Voice recorded! (Type a summary in text box for analysis.)")
        st.markdown('</div>', unsafe_allow_html=True)

    if st.button("ðŸ” Analyze & Get Support", use_container_width=True) and input_text.strip():
        with st.spinner("Analyzing..."):
            ...
            if response:
                ...
                fallback_text = f"""
### AI Support (Fallback)
- **Validation:** I hear that you're saying: "{input_text[:120]}..." â€” that sounds distressing and important.
- **Immediate steps (tailored):**
  ...
  3. Reach out to one trusted person with this exact line: "I need to talk â€” I haven't been okay lately."
- **12â€“24 hour plan:** sleep hygiene, short walk outside, limit caffeine, connect with someone.
...
with col2:
    ...
    st.markdown('<div class="emergency-banner">ðŸš¨ IN CRISIS? CALL KIRAN 1800-599-0019 ðŸš¨</div>', unsafe_allow_html=True)
