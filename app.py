...
def safe_generate(prompt, fallback="Unable to generate response."):
    """
    Try PRIMARY_MODEL (gemini-2.5-flash) first.
    If it fails for any reason, silently retry with FALLBACK_MODEL (gemini-1.5-flash).
    Updates st.session_state['active_model'] so the sidebar can show which model is live.
    """
...
with st.sidebar:
    ...
    if "error" in active:
        st.warning(f"⚠️ Model: {active}")
    elif active == FALLBACK_MODEL:
        st.info(f"🔄 Using fallback: `{FALLBACK_MODEL}`")
    else:
...
with tab_voice:
    ...
    try:
        ...
        st.error("Install `audio-recorder-streamlit` to use voice input: `pip install audio-recorder-streamlit`")
        audio_data = None
