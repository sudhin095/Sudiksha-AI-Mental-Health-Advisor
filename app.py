...
# ─── Model Chain ──────────────────────────────────────────────────────────────
# Text/Gemini priority: 1.5-flash (PRIMARY) → 2.5-flash (FALLBACK) → 1.5-flash-8b (LAST RESORT)
GEMINI_TEXT_CHAIN = [
    "gemini-1.5-flash",   # PRIMARY  — 1,500 RPD free
    "gemini-2.5-flash",   # FALLBACK — 250 RPD free
    "gemini-1.5-flash-8b" # LAST RESORT — 1,500 RPD free
]
OPENROUTER_TEXT_MODEL = "meta-llama/llama-3.3-70b-instruct:free"
GROQ_TEXT_MODELS = [
    "llama-3.1-8b-instant",    # 14,400 RPD free — fastest
    "llama-3.3-70b-versatile", # 1,000  RPD free — best quality
]

# ─── Secret Reader (Streamlit Cloud + localhost) ──────────────────────────────
...
# ─── Single Gemini model call with back-off ───────────────────────────────────
def _retry_generate(model_id: str, prompt: str, max_retries: int = 3) -> str:
    """Calls one Gemini model. Quota errors skip immediately; transient errors retry."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if is_quota:
                break  # Skip immediately — move to next model
...
def gemini_text_with_fallback(prompt: str) -> tuple:
    """
    Ultra-stable text call with this priority chain:
      ...
      4. Gemini 1.5 Flash        (PRIMARY  — 1,500 RPD)
      5. Gemini 2.5 Flash        (FALLBACK — 250 RPD)
      6. Gemini 1.5 Flash-8B     (LAST RESORT)
    ...
    # 4, 5, 6 — Gemini: 1.5-flash → 2.5-flash → 1.5-flash-8b
    for model_id in GEMINI_TEXT_CHAIN:
        try:
...
# ─── AI-Enhanced Tips (falls back to static tips silently if AI fails) ────────
def get_ai_tips(user_text: str, score: int) -> tuple:
    """Returns (tips_text, model_used). Falls back to get_tips() if all models fail."""
    cache_key = _make_cache_key(f"tips:{score}:{user_text[:200]}")
    cached = _cache_get(cache_key)
    if cached:
        return cached

    level = "mild" if score < 40 else "moderate" if score < 70 else "high"
    prompt = (
        ...
        "Do NOT mention diagnosis, medication, or act as a doctor. Be compassionate and human."
    )
    try:
        ...
        return get_tips(score), "local"  # Silent fallback — user sees original tips
    ...
    if file:
        ...
        if st.button("Transcribe & Analyze Audio"):
            ...
            if err:
                ...
                tips, model_used = get_ai_tips(text, score)
